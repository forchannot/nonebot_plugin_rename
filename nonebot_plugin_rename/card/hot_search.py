...
async def hot(num: int) -> str:
    URLSTART = "https://hot.zhenxun.buzz" if env_config.hot_search_url == 1 else "https://tenapi.cn/v2"
    URL = URL1 if env_config.hot_search_url == 1 else URL2
    async with httpx.AsyncClient(follow_redirects=True) as client:
        try:
            res = await client.get(f"{URLSTART}/{URL[str(num)][0]}")
            if res.status_code != 200:
                logger.warning(f"获取{URL[str(num)][1]}热搜失败: {res.status_code}，请尝试更换API后重启")
                return "热搜获取失败"
            data = res.json()["data"]
            result = random.choice(data)["title"] if env_config.hot_search_url == 1 else random.choice(data)["name"]
